...
def ga(query_layer,key_layer,value_layer,dropout,weight):
        ...
        if(glo.debug):###############
            print(torch.any(torch.isnan(key_layer)))
            print(torch.any(torch.isnan(query_layer)))
            print(torch.any(torch.isnan(value_layer)))
        ...
        if(glo.debug):##################
            print(torch.any(torch.isnan(attention_scores)))
        ...
        if(scatter_before==True):
            ...
            if(glo.debug):##################
                print('attn',attention_scores.max())
            fm=attn*cnt.unsqueeze(2)
            fm=torch.sum(fm,3).unsqueeze(2)
            if(glo.debug):##################
                print('fm',fm.size(),fm.min())
                print('attn',attn.size(),attn.max())
            attention_probs=(attn.permute(0,1,3,2)/fm).permute(0,1,3,2)
            torch.cuda.synchronize()
            speed.softmax+=time.time()-st
            if(glo.debug):##################
                print(torch.any(torch.isnan(attention_probs)))
        ...
        if(glo.debug):##################
            print(torch.any(torch.isnan(context_layer)))
            print()

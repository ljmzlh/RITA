...
from utils import glo, my_save, speed, my_restore
...
def dev_epoch(args,dataset_dev,model,writer):
    ...
    print('Min validation loss:',dev_min)
    ...
    if(rec.get('acc')!=None):
        ...
        print('Max validation accuracy:',dev_acc)
...
def train_epoch(now_epoch,dataset_train,dataset_dev,args,
    ...
    global global_sample,acc_loss,last_N,last_log,last_dev,pre_grp_n
    global training_time,dev_time,p_data_time,train_epoch_time
    global bsz_it
    ...
    while(batch_st<dataset_train.len):
        ...
        if(tolerance):
            try:
                ...
                print(glo.batch_size,'OOM fail -- updating batch size')
        ...
        if((global_sample-last_log)>args.log_sample):
                ...
                if(args.device=='cuda' and args.N_policy=='auto'):
...
def train(dataset_train,dataset_dev,args,model,writer,device):
    ...
    print('Dev per',args.dev_sample,'samples')

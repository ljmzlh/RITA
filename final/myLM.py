...
class BERT(nn.Module):
    ...
    def __init__(self,hidden_siz,num_hidden_layers,num_attention_heads,intermediate_siz,num_class,device,args):
        ...
        self.bert = myBertModel(config,args.pos_mode,args).to(device)
    ...
    def classify(self,input,args):
        ...
        if(args.cls_method=='cls'):
            hid=hidden[:,0,:]
        elif(args.cls_method=='mean'):
            hid=hidden[:,1:,:].mean(1)
        elif(args.cls_method=='max'):
            hid=hidden[:,1:,:].max(1).values
